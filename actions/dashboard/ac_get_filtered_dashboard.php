...
try {
    ...
    $sql_totals = "SELECT asset_type, COUNT(*) as total FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] WHERE is_active = 1 GROUP BY asset_type";
    ...
    while ($row = sqlsrv_fetch_array($res_totals, SQLSRV_FETCH_ASSOC)) {
        ...
        if (isset($response[$type]))
            $response[$type]['total'] = (int) $row['total'];
    }
    ...
    if ($hasDateFilter) {
        ...
        $sql_stats = "SELECT m.id, m.asset_type,
                        CASE 
                            WHEN EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id = m.id AND t.inspection_date >= ? AND t.inspection_date <= ? AND " . ($at == 'APAR' ? $apar_ok_check : $hydrant_ok_check) . ") THEN 'OK'
                            WHEN EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id = m.id AND t.inspection_date >= ? AND t.inspection_date <= ? AND NOT " . ($at == 'APAR' ? $apar_ok_check : $hydrant_ok_check) . ") THEN 'Abnormal'
                            WHEN EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] l WHERE l.asset_id = m.id AND l.created_at <= ? AND (l.verified_at IS NULL OR l.verified_at >= ?)) THEN 'Abnormal'
                            ELSE 'Proses'
                        END as status_period
                      FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] m WHERE m.is_active = 1";
        ...
        $sql_abn = "SELECT m.asset_type, COUNT(DISTINCT m.id) as count
                    ...
                    WHERE m.is_active = 1 AND (
                        EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id = m.id AND CAST(t.inspection_date AS DATE) BETWEEN ? AND ? AND (
                            (m.asset_type = 'APAR' AND NOT $apar_ok_check) OR (m.asset_type = 'HYDRANT' AND NOT $hydrant_ok_check)
                        )) OR
                        EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] l WHERE l.asset_id = m.id AND CAST(l.created_at AS DATE) <= ? AND (l.verified_at IS NULL OR CAST(l.verified_at AS DATE) >= ?))
        ...
        $sqlMarkers = "SELECT 
                ...
                CASE 
                    WHEN EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id = m.id AND CAST(t.inspection_date AS DATE) BETWEEN ? AND ? AND (
                        (m.asset_type = 'APAR' AND NOT $apar_ok_check) OR (m.asset_type = 'HYDRANT' AND NOT $hydrant_ok_check)
                    )) OR EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] l WHERE l.asset_id = m.id AND CAST(l.created_at AS DATE) <= ? AND (l.verified_at IS NULL OR CAST(l.verified_at AS DATE) >= ?)) THEN 'Abnormal'
                    WHEN EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id = m.id AND CAST(t.inspection_date AS DATE) BETWEEN ? AND ? AND (
                        (m.asset_type = 'APAR' AND $apar_ok_check) OR (m.asset_type = 'HYDRANT' AND $hydrant_ok_check)
                    )) THEN 'OK'
                    ELSE 'Proses'
        ...
        foreach (['apar', 'hydrant'] as $t) {
            $ut = strtoupper($t);
            $response[$t]['ok'] = (int) sqlsrv_fetch_array(sqlsrv_query($koneksi, "SELECT COUNT(*) FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] WHERE asset_type='$ut' AND status='OK' AND is_active=1"))[0];
            $response[$t]['abnormal'] = (int) sqlsrv_fetch_array(sqlsrv_query($koneksi, "SELECT COUNT(*) FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] WHERE asset_type='$ut' AND status='NG' AND is_active=1"))[0];

            $now_m = date('m');
            $now_y = date('Y');
            $response[$t]['proses'] = (int) sqlsrv_fetch_array(sqlsrv_query($koneksi, "SELECT COUNT(*) FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] m WHERE asset_type='$ut' AND is_active=1 AND NOT EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id=m.id AND MONTH(t.inspection_date)=$now_m AND YEAR(t.inspection_date)=$now_y)"))[0];
        }

        $sqlMarkers = "SELECT id, asset_code as code, asset_type as jenis, x_coordinate, y_coordinate, area,
                        CASE 
                            WHEN status = 'NG' THEN 'Abnormal'
                            WHEN EXISTS (SELECT 1 FROM [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] t WHERE t.asset_id=m.id AND MONTH(t.inspection_date)=MONTH(GETDATE()) AND YEAR(t.inspection_date)=YEAR(GETDATE())) THEN 'OK'
                            ELSE 'Proses'
                        END as status_badge
                       FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] m
