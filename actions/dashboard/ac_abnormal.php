...
try {
    ...
    if ($action === 'start_progress') {
        ...
        $sql = "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] 
        ...
        if ($stmt === false)
            throw new Exception('Gagal memulai proses.');
    ...
    if ($action === 'update_detail') {
        ...
        $sql = "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] SET 
                finding_desc = ?, 
                countermeasure = ?, 
                due_date = ?, 
                verified_by = ?, 
        ...
        if ($stmt === false)
            throw new Exception('Gagal mengupdate detail kasus.');
    ...
    if ($action === 'update_status') {
        ...
        $sql = "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] SET repair_status = ?, updated_at = GETDATE()";
        $params = [$new_status];
        if ($photoPath) {
            $sql .= ", repair_photo = ?";
            $params[] = $photoPath;
    ...
    if ($action === 'verify_case') {
        ...
        // 1. Get Asset ID
        $get_asset = sqlsrv_query($koneksi, "SELECT asset_id FROM [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] WHERE id = ?", [$id]);
        ...
        $check_others = sqlsrv_query($koneksi, "SELECT COUNT(*) as open_issues FROM [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] WHERE asset_id = ? AND repair_status <> 'Verified'", [$asset_id]);
        $others_row = sqlsrv_fetch_array($check_others, SQLSRV_FETCH_ASSOC);

        if ($others_row['open_issues'] == 0) {
            // All verified, unit is OK
            sqlsrv_query($koneksi, "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] SET status = 'OK' WHERE id = ?", [$asset_id]);
        }
