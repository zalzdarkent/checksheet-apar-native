...
try {
    ...
    $sql_trans = "INSERT INTO [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] (" . implode(',', $cols) . ") VALUES (" . implode(',', $placeholders) . "); SELECT SCOPE_IDENTITY() AS last_id;";
    ...
    sqlsrv_query($koneksi, "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] SET last_inspection_date = ? WHERE id = ?", [$inspection_date, $equipment_id]);

    // 4. Handle Abnormal Findings (LINES)
    if (!empty($abnormal_items)) {
        sqlsrv_query($koneksi, "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] SET status = 'NG' WHERE id = ?", [$equipment_id]);

        // Fetch PIC EMPID from Master
        $sql_pic = "SELECT pic_empid FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] WHERE id = ?";
