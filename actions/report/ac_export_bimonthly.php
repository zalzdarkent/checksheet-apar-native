...
try {
    ...
    if ($stmt === false) throw new Exception("Database Error: " . print_r(sqlsrv_errors(), true));
    ...
    if ($format === 'excel') {
        ...
        foreach ($periods as $period) $header[] = $period['name'];
        ...
        foreach ($equipment_raw as $item) {
            ...
            if (!isset($equipment_data[$key])) {
                $equipment_data[$key] = array(
                    'no' => $no++, 'code' => $item['code'], 'area' => $item['area'], 
                ...
                foreach ($periods as $idx => $period) {
                    if (in_array($month, $period['months'])) {
                        ...
                        foreach ($check_items as $ci) {
                            if (isset($item[$ci]) && $item[$ci] != 1) { $all_ok = false; break; }
                        }
        ...
        foreach ($equipment_data as $row_data) {
            ...
            foreach ($periods as $idx => $period) $row[] = $row_data['periods'][$idx];
            fputcsv($output, $row);
        }
        fclose($output);
        ...
        foreach ($periods as $p) $pdf->Cell(25, 7, $p['name'], 1, 0, 'C', true);
        $pdf->Ln();
        ...
        foreach ($equipment_raw as $item) {
            ...
            if (!isset($equipment_data[$key])) {
                $equipment_data[$key] = array(
                    'no' => $no++, 'code' => $item['code'], 'area' => $item['area'],
                    'location' => $item['location'], 'periods' => array_fill_keys(array_keys($periods), '-')
                );
            }
            if (!empty($item['inspection_date'])) {
                $month = $item['inspection_date']->format('n');
        ...
        foreach ($equipment_data as $item) {
            ...
            foreach ($periods as $idx => $period) $pdf->Cell(25, 6, $item['periods'][$idx], 1, 0, 'C', $fill);
            $pdf->Ln();
        }
