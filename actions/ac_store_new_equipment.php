...
try {
    ...
    $check_sql = "SELECT COUNT(*) as count FROM [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] WHERE asset_code = ?";
    ...
    $sql_master = "INSERT INTO [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] 
    ...
    if ($stmt_master === false)
        throw new Exception("Gagal simpan master: " . print_r(sqlsrv_errors(), true));
    ...
    $sql_trans = "INSERT INTO [PRD].[dbo].[SE_FIRE_PROTECTION_TRANS] (" . implode(',', $cols) . ") 
                 OUTPUT INSERTED.id VALUES (" . implode(',', $placeholders) . ")";
    $stmt_trans = sqlsrv_query($koneksi, $sql_trans, $params_trans);
    if ($stmt_trans === false)
        throw new Exception("Gagal simpan inspeksi: " . print_r(sqlsrv_errors(), true));

    $trans_row = sqlsrv_fetch_array($stmt_trans, SQLSRV_FETCH_ASSOC);
    $trans_id = $trans_row['id'];

    sqlsrv_query($koneksi, "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] SET last_inspection_date = ? WHERE id = ?", [$now, $new_asset_id]);

    // 4. Abnormal Cases (LINES)
    if (!empty($abnormal_items)) {
        sqlsrv_query($koneksi, "UPDATE [PRD].[dbo].[SE_FIRE_PROTECTION_MASTER] SET status = 'NG' WHERE id = ?", [$new_asset_id]);
        ...
        $sql_line = "INSERT INTO [PRD].[dbo].[SE_FIRE_PROTECTION_LINES] 
