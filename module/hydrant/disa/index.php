<?php
$area = 'Disa';
include(__DIR__ . '/../../../actions/disa/ac_get_data_hydrant.php');
...
<div class="page-inner">
    ...
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Hydrant Management - Disa</h3>
        </div>
        <div class="ms-md-auto py-2 py-md-0 d-flex gap-2">
            ...
            <a href="?page=hydrant-create&area=Disa" class="btn btn-primary btn-round">
    ...
    <div id="hydrant-container" class="hydrant-card-container">
        ...
        <?php else: ?>
            <?php foreach ($hydrant_data as $item): ?>
                ...
                <div class="hydrant-card <?php echo $item['is_active'] == 0 ? 'inactive' : ''; ?>" data-id="<?php echo $item['id']; ?>">
                    ...
                    <div class="hydrant-info">
                        ...
                        <div class="hydrant-code"><?php echo $item['code']; ?></div>
                        <div class="hydrant-location">
                            <i class="fas fa-map-marker-alt"></i> <?php echo $item['location']; ?>
                        </div>
                        <div class="hydrant-details text-left">
...
<script>
    $(document).ready(function () {
        ...
        function toggleSelectedActions() {
            ...
            if (selectedCount > 0) {
                ...
                selectedCheckboxes.each(function () {
                    const card = $(this).closest('.apar-card, .hydrant-card');
        ...
        $('#hydrant-container').on('change', '.item-checkbox', function () {
            const card = $(this).closest('.hydrant-card');
            if ($(this).is(':checked')) {
        ...
        $('#hydrant-container').on('click', '.hydrant-card', function (e) {
        ...
        $('#print-qr-selected').on('click', function (e) {
            ...
            const selectedIds = $('.item-checkbox:checked').map(function () {
                return $(this).closest('.hydrant-card').data('id');
            }).get();
        ...
        $('#hydrant-container, #apar-container').on('click', '.btn-print-qr', function (e) {
            e.stopPropagation();
        ...
        $('#hydrant-container, #apar-container').on('click', '.status-toggle-btn', function (e) {
            e.stopPropagation();
            const btn = $(this);
            ...
            swal({
                ...
                if (willUpdate) {
                    $.ajax({
                        ...
                        type: 'POST',
                        data: { type: 'hydrant', ids: id, status: newStatus },
                        dataType: 'json',
                        success: function (response) {
                            if (response.success) {
                                const card = btn.closest('.apar-card, .hydrant-card');
                                if (newStatus == 0) {
            ...
            swal({
                ...
                if (willUpdate) {
                    $.ajax({
                        url: 'actions/ac_toggle_active.php',
                        type: 'POST',
                        data: { type: 'hydrant', ids: selectedIds.join(','), status: newStatus },
                        dataType: 'json',
                        success: function (response) {
                            if (response.success) {
                                $.notify({
        ...
        function createCardHtml(item) {
            ...
            return `
                <div class="hydrant-card apar-card ${inactiveClass}" data-id="${item.id}">
                    <input type="checkbox" class="card-checkbox item-checkbox">
                    ...
                    <div class="hydrant-info">
                        ...
                        <div class="hydrant-code">${item.code}</div>
                        <div class="hydrant-location">
        ...
        function loadMore() {
            ...
            $.ajax({
                url: 'actions/disa/ac_get_data_hydrant.php',
                type: 'GET',
                ...
                success: function (data) {
                    if (data && data.length > 0) {
                        ...
                        const currentCount = $('.hydrant-card').length;
                        ...
                        hasMore = false;
                        if (currentPage === 1) {
                            $('#hydrant-container').html('<div id="no-data-msg" class="text-center w-100 py-5" style="grid-column: 1 / -1; color: #a0a0a0; font-size: 1.2rem;">Data tidak ditemukan</div>');
